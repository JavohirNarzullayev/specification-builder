...
import static uz.narzullayev.javohir.dto.TypeSearch.getType;
...
public class SearchSpecification<T> implements Specification<T> {
    ...
    public static <T> SearchSpecification<T> type(Class<T> tClass) {
        return new SearchSpecification<>();
    }

    public SearchSpecification<T> conditions(List<SearchCriteria> conditions) {
    ...
    private Predicate getPredicate(SearchCriteria param, CriteriaBuilder builder, Root<T> root, CriteriaQuery<?> query) {
        ...
        if (isJoinColumn) return join(param, builder, root, query);
    ...
    private Predicate getPredicateIn(SearchCriteria param, CriteriaBuilder builder, Root<T> root) {
        ...
        var type = getType(root.getJavaType(), param.getKey());
        return switch (type) {
    ...
    private Predicate greaterThan(SearchCriteria param, CriteriaBuilder builder, Root<T> root) {
        ...
        switch (type) {
            ...
            case DATE -> {
                var date = toDate(param.getValue());
    ...
    private Predicate lessThanOrEqual(SearchCriteria param, CriteriaBuilder builder, Root<T> root) {
        ...
        return switch (type) {
            ...
            default -> throw new SearchException("Less than or equal not support type " + type);
        };
    }
    ...
    private Predicate startWith(SearchCriteria param, CriteriaBuilder builder, Root<T> root) {
        ...
        switch (type) {
            ...
            default -> throw new SearchException("Start with not support type " + type);
        }
    }
                ...
                if (paramType.equals(HashMap.class)) {
                    yield builder.or(
                            ...
                            builder.like(builder.lower(builder.function(
                                    "jsonb_extract_path_text",
                                    String.class,
                                    root.get(param.getKey()), builder.literal("kar"))), "%" + value.toLowerCase() + "%"));
                }
