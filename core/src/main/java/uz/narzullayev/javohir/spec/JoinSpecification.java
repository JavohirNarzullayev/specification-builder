package uz.narzullayev.javohir.spec;
...
import static uz.narzullayev.javohir.util.ReflectionUtils.*;
...
public class JoinSpecification {
    ...
    private static Predicate getPredicateIn(SearchCriteria param,
                                            CriteriaBuilder builder,
                                            Join<Object, Object> joinPath,
                                            String[] key,
                                            Object value) {
        ...
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
        return switch (type) {
            ...
            default -> throw new SearchException("In not support for type " + type);
        };
    }

    private Predicate greaterThan(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        ...
        switch (type) {
            ...
            case DATE -> {
                var date = toDate(value.toString());
            ...
            default -> throw new SearchException("Greater than not support type " + type);
        }
    }

    private Predicate greaterThanOrEqual(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        if (isJson(joinPath.getJavaType(), key[0])) {
            throw new SearchException("Json type not support for less than");
        }
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
        switch (type) {
            ...
            default -> throw new SearchException("Greater or equal than not support type " + type);
        }
    }


    private Predicate lessThan(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        if (isJson(joinPath.getJavaType(), key[0])) {
            throw new SearchException("Json type not support for less than");
        }
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
        switch (type) {
            ...
            default -> throw new SearchException("Less than not support type " + type);
        }
    }

    private Predicate lessThanOrEqual(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        if (isJson(joinPath.getJavaType(), key[0])) {
            throw new SearchException("Json type not support for less than");
        }
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
    ...
    private Predicate endWith(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
        switch (type) {
            case STRING -> {
            ...
            default -> throw new SearchException("End with not support type " + type);
        }
    }

    private Predicate startWith(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object value) {
        var type = TypeSearch.getType(joinPath.getJavaType(), key[0]);
        switch (type) {
            case STRING -> {
                return builder.like(builder.lower(joinPath.get(key[0])), value.toString().toLowerCase() + "%");
    ...
    private Predicate containWith(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object val) {
        ...
        switch (type) {
            ...
            default -> throw new SearchException("Contains  not support type " + type);
        }
    }

    @SuppressWarnings("all")
    private Predicate getPredicateOnNotEqual(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] key, Object val) {
        ...
        switch (type) {
                ...
                if (paramType.equals(HashMap.class)) {
                    return builder.and(
                            ...
                            builder.notLike(builder.lower(builder.function(
                                    "jsonb_extract_path_text",
                                    String.class,
                                    joinPath.get(keys[0]), builder.literal("kar"))), "%" + value.toLowerCase() + "%"));
                }
            ...
            default -> throw new SearchException("Not support type " + type);
        }
    }


    @SuppressWarnings("all")
    private Predicate getPredicateOnEqual(SearchCriteria param, CriteriaBuilder builder, Join<Object, Object> joinPath, String[] keys, Object val) {
        var paramType = joinPath.get(keys[0]).getJavaType();
        if (val == null) {
            return builder.isNull(joinPath.get(keys[0]));
        }
        var type = TypeSearch.getType(joinPath.getJavaType(), keys[0]);
        return switch (type) {
            case NUMBER -> {
            ...
            default -> throw new SearchException("NOT SUPPORTED FOR TYPE" + type);
        };
    }
}
